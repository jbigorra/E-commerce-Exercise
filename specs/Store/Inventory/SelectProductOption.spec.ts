import { SelectProductOption, SelectProductOptionCommand } from "../../../src/Store/Inventory/Actions/SelectProductOption"
import { ProductRepository } from "../../../src/Store/Inventory/Infrastructure/InMemoryInventory";
import { Product, ProductOption } from "../../../src/Store/Inventory/Core/Entities";
...
describe("SelectProductOption", () => {
  ...
  it("should return the product with the selected option", () => {
        ...
        expectSuccess<Product>(actionResult, {
            id: 2,
            availableOptions: (opts: ProductOption[]) => expect(opts).toEqual(
              expect.arrayContaining([expect.objectContaining({ id: 1 }), expect.objectContaining({ id: 2 }), expect.objectContaining({ id: 3 })])
            ),
            selectedOptions: (opts: ProductOption[]) => expect(opts).toEqual(
