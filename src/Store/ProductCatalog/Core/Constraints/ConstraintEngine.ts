...
class DefaultConstraintHandler implements PriorityConstraintStrategy {
  ...
  apply(constraint: Constraint, context: ConstraintContext): Result<void> {
    return Result.error(new Error(`No handler for constraint type: ${constraint.type}`));
  }
}

export class ConstraintEngine {
  readonly defaultHandler: PriorityConstraintStrategy = new DefaultConstraintHandler();
  ...
  applyConstraints(constraints: Constraint[], context: ConstraintContext): Result<void> {
    for (const handler of this.sortedHandlers) {
      const applicableConstraints = constraints.filter((c) => handler.canHandle(c));
    ...
    const unhandledConstraints = constraints.filter((c) => !this.sortedHandlers.some((h) => h.canHandle(c)));
